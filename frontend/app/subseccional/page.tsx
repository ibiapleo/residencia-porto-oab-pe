...
export default function SubseccionalPage() {
  ...
  const { data, isLoading, error, isEmpty, refetch } = useSubseccionais({
  ...
  const handleDelete = async (id: string) => {
    setIsDeleting(true);
    try {
      await excluirSubseccional(id);
      toast({
        title: "Sucesso",
        description: "Subseccional excluída com sucesso",
      });
      refetch();
    } catch (error) {
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Não foi possível excluir a subseccional",
        variant: "destructive",
      });
    } finally {
      setIsDeleting(false);
  ...
  const columns: ColumnDef<SubseccionalResponseDTO>[] = [
    {
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      permanentemente a subseccional e removerá os dados de
                      nossos servidores.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
    ...
    return (
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <div className="flex items-center justify-between">
          <h2 className="text-3xl font-bold tracking-tight">Subseccionais</h2>
          <Button asChild className="bg-secondary hover:bg-secondary/90">
            <Link href="/subseccional/new">
              <Plus className="mr-2 h-4 w-4" /> Nova Subseccional
            </Link>
          </Button>
        </div>
        <div className="rounded-md border border-red-200 bg-red-50 p-4">
          <p className="text-red-600">
            Erro ao carregar subseccionais: {error.message}
          </p>
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center space-y-4 rounded-md border p-8 text-center">
          ...
          <h3 className="text-xl font-semibold">
            Nenhuma subseccional encontrada
          </h3>
          <p className="text-muted-foreground">
            Você ainda não cadastrou nenhuma subseccional
          </p>
          <Button asChild>
            <Link href="/subseccional/new">
              <Plus className="mr-2 h-4 w-4" /> Criar primeira subseccional
            </Link>
          </Button>
        </div>
      ) : (
