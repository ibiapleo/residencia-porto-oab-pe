...
import { excluirDemonstrativo } from "@/services/demonstrativoService";
...
export default function DemonstrativoPage() {
  ...
  // Hook personalizado para buscar demonstrativos
  const { data, isLoading, error, isEmpty, refetch } = useDemonstrativos({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Demonstrativo excluído com sucesso",
      });
      refetch();
    } catch (error) {
      toast({
        title: "Erro",
        description: error instanceof Error ? error.message : "Não foi possível excluir o demonstrativo",
        variant: "destructive",
      });
    } finally {
      setIsDeleting(false);
  ...
  const columns: ColumnDef<DemonstrativoResponseDTO>[] = [
    {
      accessorKey: "nome",
      header: "Nome",
      enableSorting: true,
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      o demonstrativo e removerá os dados de nossos servidores.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
    ...
    return (
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <div className="flex items-center justify-between">
          <h2 className="text-3xl font-bold tracking-tight">Demonstrativos</h2>
          <Button asChild className="bg-secondary hover:bg-secondary/90">
            <Link href="/demonstrativos/new">
              <Plus className="mr-2 h-4 w-4" /> Novo Demonstrativo
            </Link>
          </Button>
        </div>
        <div className="rounded-md border border-red-200 bg-red-50 p-4">
          <p className="text-red-600">
            Erro ao carregar demonstrativos: {error.message}
          </p>
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center space-y-4 rounded-md border p-8 text-center">
          ...
          <h3 className="text-xl font-semibold">Nenhum demonstrativo encontrado</h3>
          <p className="text-muted-foreground">
            Você ainda não cadastrou nenhum demonstrativo
          </p>
          <Button asChild>
            <Link href="/demonstrativos/new">
              <Plus className="mr-2 h-4 w-4" /> Criar primeiro demonstrativo
            </Link>
          </Button>
        </div>
      ) : (
